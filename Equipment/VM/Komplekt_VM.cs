...
namespace Equipment.VM
{
    public class Komplekt_VM: Base_VM
    {
        ...
        Collection<Status_M> statuses;
        public Collection<Status_M> Statuses//Список статусов
        {
            get => statuses;
            set
            {
        ...
        void To_do()
        {
            using (EqContext ec = new EqContext())
            {
                ...
                KomplektTable = new ObservableCollection<Komplekt_M>(i.Include(x=>x.Status).OrderBy(x=>x.Id));
                NewItem.Status = null;
        ...
        public RelayCommand NextPage
        {
            get
            {
                return nextPage ??= new RelayCommand(o =>
                {
                    CurrentPage += 1;
                    To_do();
                }, o => CurrentPage < Convert.ToInt32(AllPage));
            }
        }
    }
}
